...
├── base-debs/           # Generic Ubuntu 24.04 packages for airgapped mode
...
artifacts/nkp-2.18/base-debs/*.deb
artifacts/nkp-2.18/docker-debs/*.deb
artifacts/nkp-2.18/tools/kubectl-linux-amd64
```

Download the generic Ubuntu, Docker and kubectl artifacts on an Internet-connected
Ubuntu 24.04 AMD64 staging host:

```bash
chmod +x scripts/download-generic-airgap-artifacts.sh
./scripts/download-generic-airgap-artifacts.sh
```

The helper downloads Python, OpenSSH, sudo, CA/curl/GnuPG/archive utilities,
Docker 27.4 or newer with dependency closures, and kubectl 1.35.x. It verifies
the kubectl checksum and writes `artifacts/nkp-2.18/generic-artifacts.sha256`.
It deliberately does not download Nutanix-licensed or large files.
...
- The base DEB directory includes the Ubuntu 24.04 packages and dependencies generated by the helper.
